 ...
 * 1. EXTRAE el campo Linea de PrecioUNew.txt para todos los códigos.
 * 2. MIGRA registros del formato antiguo (sin Linea) al nuevo (con Linea).
 * 3. ACTUALIZA precios de registros existentes cuyo Precio A difiera en ProductNew.txt.
 *    - Mantiene las proporciones B/A y C/A originales de cada producto.
 * 4. AGREGA los registros nuevos (presentes en PrecioUNew pero no en Inserciones).
 *
 * Formato antiguo: ('codigo', 'producto', 'medida', 'precioC', 'precioB', 'precioA')
 * Formato nuevo:   ('codigo', 'producto', 'medida', 'linea',   'precioC', 'precioB', 'precioA')
 */
...
// ============================================================
// 2. Extraer campo Linea de PrecioUNew.txt para TODOS los códigos
//    Formato PrecioUNew: ('codigo', 'producto', 'marca', 'linea', 'grupo', ...)
// ============================================================
const regexLinea = /^\s+\('(\d+)',\s*'[^']*',\s*'[^']*',\s*'([^']*)'/gm;
const lineaData = {};
while ((m = regexLinea.exec(newFile)) !== null) {
  lineaData[m[1]] = m[2].replace(/'/g, "''");
}
console.log('Datos de Linea cargados de PrecioUNew.txt:', Object.keys(lineaData).length);

// ============================================================
// 3. Leer archivo principal y actualizar encabezado INSERT
// ============================================================
let content = fs.readFileSync(mainPath, 'utf8');

const OLD_HEADER = 'INSERT INTO "Precio_Unitario" ("Código", "Producto", "Medida", "Precio C", "Precio B", "Precio A") VALUES';
const NEW_HEADER = 'INSERT INTO "Precio_Unitario" ("Código", "Producto", "Medida", "Linea", "Precio C", "Precio B", "Precio A") VALUES';
if (content.includes(OLD_HEADER)) {
  content = content.replace(OLD_HEADER, NEW_HEADER);
  console.log('Encabezado actualizado al nuevo formato con columna Linea.');
}

// ============================================================
// 4. MIGRACIÓN: convertir registros del formato antiguo (sin Linea) al nuevo
//    Formato antiguo: ('codigo', 'producto', 'medida', 'precioC', 'precioB', 'precioA')
//    Distinción: campo 4 es numérico (solo dígitos y punto) → formato antiguo.
// ============================================================
const regexOldFormat = /^(\s+)\('(\d+)',(\s*'[^']*'),(\s*'[^']*'),(\s*'([\d.]+)'),(\s*'([\d.]+)'),(\s*'([\d.]+)')\)([,;])/gm;

let migrados = 0;
content = content.replace(regexOldFormat, (match, indent, codigo, producto, medida, _cStr, precioC, _bStr, precioB, _aStr, precioA, terminator) => {
  const linea = lineaData[codigo] || 'GENERAL';
  migrados++;
  return indent + "('" + codigo + "'," + producto + "," + medida + ", '" + linea + "', '" + precioC + "', '" + precioB + "', '" + precioA + "')" + terminator;
});
console.log('Registros migrados al nuevo formato (con Linea):', migrados);

// ============================================================
// 5. ACTUALIZACIÓN DE PRECIOS en registros de nuevo formato
//    Formato nuevo: ('codigo', 'producto', 'medida', 'linea', 'precioC', 'precioB', 'precioA')
//    Se preservan las proporciones C/A y B/A originales de cada producto.
// ============================================================
const regexNewFormat = /^(\s+)\('(\d+)',(\s*'[^']*'),(\s*'[^']*'),(\s*'[^']*'),(\s*'([\d.]+)'),(\s*'([\d.]+)'),(\s*'([\d.]+)')\)([,;])/gm;

let actualizados = 0;
let sinCambio = 0;

content = content.replace(regexNewFormat, (match, indent, codigo, producto, medida, lineaStr, _cStr, precioC, _bStr, precioB, _aStr, precioA, terminator) => {
  ...
  actualizados++;
  return indent + "('" + codigo + "'," + producto + "," + medida + "," + lineaStr + ", '" + fmtStr(newC) + "', '" + fmtStr(newB) + "', '" + fmtStr(newPrecioA) + "')" + terminator;
});
...
// ============================================================
// 6. Agregar registros NUEVOS (en PrecioUNew pero no en Inserciones)
// ============================================================
...
// La linea de cada nuevo código se obtiene de lineaData (ya cargado en sección 2)
...
if (sortedNew.length > 0) {
  const rawLines = sortedNew.map((codigo) => {
    ...
    return "    ('" + codigo + "', '" + producto + "', 'UNIDAD', '" + linea + "', '" + pA + "', '" + precioB + "', '" + pA + "')";
  });
...
// ============================================================
// 8. Reporte: registros sin precio disponible en ProductNew
// ============================================================
